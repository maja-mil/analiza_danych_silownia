...
Poniżej przedstawiamy podstawową charakterystykę próby oraz statystyki opisowe dla zmiennych liczbowych i jakościowych na danych po imputacji i korekcie błędów.
...
Zbiór danych po imputacji i korekcie błędów zawiera 973 obserwacje oraz 30 zmiennych. Wśród nich znajdują się zarówno zmienne opisujące użytkowników i trening (m.in. wiek, płeć, masa ciała, wzrost, wskaźniki tętna, czas sesji, spalone kalorie, typ treningu, poziom doświadczenia), jak i zmienne techniczne związane z imputacją braków danych. Zmienne z końcówką \_imp (np. Age_imp, Workout_Type_imp) mają postać logiczną TRUE/FALSE i informują, czy dana wartość w danym wierszu została uzupełniona metodą imputacji.

W analizie opisowej skupiamy się na zmiennych merytorycznych (charakteryzujących użytkowników i trening), natomiast zmienne \_imp traktujemy pomocniczo – służą do kontroli procesu przygotowania danych, a nie do opisu badanej próby.

Struktura danych wskazuje, że większość zmiennych ma charakter liczbowy (np. Age, Weight, BMI, Calories_Burned, Session_Duration, wskaźniki tętna), a część to zmienne jakościowe (kategoryczne), takie jak Gender, Workout_Type oraz Experience_Level, które zostały dodatkowo przekształcone na czytelne etykiety w języku polskim.
...
Statystyki opisowe zmiennych liczbowych W tabeli zestawiono podstawowe miary położenia i zróżnicowania (średnia, odchylenie standardowe, mediana, kwartyle oraz wartości minimalne i maksymalne) dla zmiennych liczbowych w zbiorze po imputacji. Dla każdej zmiennej liczba obserwacji wynosi n = 973, co oznacza, że w danych po przygotowaniu nie występują braki w analizowanych zmiennych.

Wiek (Age) badanych mieści się w przedziale 18–59 lat, przy średniej 38,43 i medianie 39. Kwartyle (Q1 = 27, Q3 = 49) wskazują, że połowa osób ma od około 27 do 49 lat, co sugeruje przewagę osób w wieku dorosłym/średnim.

Wskaźniki tętna podczas treningu mają dość stabilne wartości w całej próbie. Średnie tętno podczas sesji (Avg_BPM) wynosi średnio 143,77 (mediana 143) i waha się od 120 do 169 uderzeń/min. Tętno maksymalne (Max_BPM) ma średnią 179,88 (mediana 180) w zakresie 160–199, a tętno spoczynkowe (Resting_BPM) średnio 62,22 (mediana 62) w zakresie 50–74. Stosunkowo niewielkie odchylenia standardowe dla tych zmiennych (ok. 7–14) pokazują, że wartości są umiarkowanie zróżnicowane między osobami.

BMI ma średnią 24,91 (mediana 24,16), a kwartyle wynoszą 20,11 i 28,56. Oznacza to, że „typowa” osoba w danych znajduje się w okolicach górnej granicy normy BMI, ale rozpiętość wartości jest duża (12,32–49,84), co wskazuje na obecność zarówno osób bardzo szczupłych, jak i osób z wysokimi wartościami BMI.

Wydatki energetyczne są wyraźnie bardziej zmienne. Spalone kalorie (Calories_Burned) mają średnią 905,42 i medianę 893, a rozrzut jest duży (min 303, max 1783, odchylenie standardowe 272,64). Kwartyle (Q1 = 720, Q3 = 1076) sugerują, że połowa treningów kończy się spaleniem od ok. 720 do 1076 kcal, ale część obserwacji reprezentuje znacznie bardziej wymagające sesje.

Dla zmiennych opisujących cechy fizyczne: średnia masa ciała (Weight) wynosi 73,85 kg (mediana 70 kg, zakres 40–129,9 kg), a średni wzrost (Height) to 1,72 m (mediana 1,71, zakres 1,50–2,00 m). Procent tkanki tłuszczowej (Fat_Percentage) ma średnią 24,98% (mediana 26,20%, zakres 10–35%), co wskazuje na umiarkowane zróżnicowanie składu ciała w próbie.

Zmienne związane z aktywnością pokazują, że przeciętna długość sesji (Session_Duration) to 1,26 h (około 1 godz. 15 min), przy zakresie od 0,5 do 2 godzin. Spożycie wody (Water_Intake) wynosi średnio 2,63 l (od 1,5 do 3,7 l). Natomiast częstotliwość treningów (Workout_Frequency) to średnio 3,32 dnia/tydzień (mediana 3, zakres 2–5), co sugeruje, że większość osób ćwiczy regularnie, kilka razy w tygodniu.
...
Płeć (Gender). W zbiorze znajduje się 511 mężczyzn (52,5%) oraz 462 kobiety (47,5%). Struktura próby jest więc stosunkowo wyrównana, z niewielką przewagą mężczyzn (różnica ok. 5 p.p.).

Typ treningu (Workout_Type). Rozkład typów treningu jest dość równomierny – żadna kategoria nie dominuje wyraźnie. Najczęściej występuje Cardio: 262 osób (26,9%) oraz trening Siłowy: 255 osoby (26,2%). Nieco rzadziej pojawiają się: Joga: 238 osób (24,5%) oraz HIIT: 218 osób (22,3%). Różnice między najczęstszą a najrzadszą kategorią wynoszą ok. 5 punktów procentowych, co sugeruje, że w danych reprezentowane są różne formy aktywności w podobnych proporcjach.

Poziom doświadczenia (Experience_Level). Najliczniejszą grupę stanowią osoby Średniozaawansowane: 406 (41,7%). Następnie osoby Początkujące: 376 (38,6%). Najmniejszą część próby tworzą osoby Zaawansowane: 191 (19,6%). Oznacza to, że większość badanych (łącznie ok. 80%) znajduje się na poziomie początkującym lub średniozaawansowanym, a osoby zaawansowane stanowią około 1/5 całej próby.
...
Na wykresach przedstawiono rozkłady trzech wybranych zmiennych liczbowych: wieku, BMI oraz liczby spalonych kalorii. Wykresy pozwalają ocenić, czy wartości są skoncentrowane wokół jednego zakresu, czy występują wartości skrajne oraz czy rozkłady są symetryczne.

Wiek (Age). Rozkład wieku jest względnie równomierny w całym zakresie obserwacji (od ok. 18 do ok. 59 lat). Nie widać jednego dominującego przedziału wiekowego – liczności w kolejnych koszykach są podobne, z niewielkimi lokalnymi „pikami”. Oznacza to, że dane obejmują szeroki przekrój osób dorosłych, a wiek nie jest w próbie silnie skoncentrowany w jednej grupie.

BMI. Rozkład BMI jest wyraźnie jednomodalny (ma jeden główny szczyt) i przypomina rozkład zbliżony do normalnego, ale z widocznym prawostronnym ogonem. Najwięcej obserwacji skupia się w okolicach wartości typowych dla zakresu normy i lekkiej nadwagi (około niskich–średnich 20-kilku), natomiast dla wyższych wartości (powyżej \~30) liczność stopniowo maleje. Jednocześnie występują pojedyncze obserwacje o bardzo wysokich wartościach BMI (do okolic 50), co wskazuje na obecność wartości skrajnych po prawej stronie rozkładu.

Spalone kalorie (Calories_Burned). Rozkład spalonych kalorii jest również jednomodalny, z największą koncentracją obserwacji w środkowym zakresie (około 700–1100 kcal). Widać jednak, że rozkład jest lekko skośny w prawo – część sesji ma bardzo wysoką liczbę spalonych kalorii (powyżej 1400–1500), ale są to już znacznie rzadsze przypadki. Z drugiej strony występują też treningi o niskim wydatku energetycznym (około 300–500 kcal), jednak są one mniej liczne niż obserwacje z centrum rozkładu. Taki kształt sugeruje, że „typowy” trening w danych ma umiarkowanie wysoką intensywność, a bardzo wymagające sesje stanowią mniejszość.

Wniosek: spośród analizowanych rozkładów największą asymetrię i obecność wartości skrajnych widać dla BMI oraz spalonych kalorii, natomiast wiek jest rozłożony bardziej równomiernie w dostępnych przedziałach.
...
Porównanie spalonych kalorii między typami treningu Na wykresie pudełkowym porównano rozkład spalonych kalorii (Calories_Burned) w zależności od typu treningu (Cardio, Siłowy, Joga, HIIT). Boxplot pokazuje medianę (linia w środku pudełka), rozstęp międzykwartylowy (pudełko) oraz wartości odstające (punkty).

We wszystkich czterech typach treningu mediany spalonych kalorii są zbliżone – różnice między grupami nie są duże i rozkłady w dużym stopniu się nakładają. Można zauważyć, że HIIT ma nieco wyższą medianę w porównaniu do pozostałych typów, natomiast Cardio i Siłowy wyglądają bardzo podobnie pod względem położenia mediany.

Pod względem zróżnicowania wyników największy rozrzut (szersze pudełko i/lub dłuższe „wąsy”) widoczny jest dla Jogi, co oznacza, że w tej kategorii spalanie kalorii jest bardziej zmienne między osobami/sesjami. W każdej grupie występują również wartości odstające po stronie wysokich kalorii (powyżej górnego wąsa), co sugeruje, że niezależnie od typu treningu zdarzają się jednostkowe sesje o bardzo dużym wydatku energetycznym.

Wniosek: typ treningu może wiązać się z niewielkimi różnicami w spalaniu kalorii (lekka przewaga HIIT), jednak w danych dominują podobne poziomy kalorii w grupach, a większa zmienność występuje wewnątrz typów treningu niż między nimi.
...
Najsilniejsze korelacje między zmiennymi liczbowymi W tabeli przedstawiono 10 najsilniejszych zależności liniowych między zmiennymi liczbowymi, mierzonych współczynnikiem korelacji Pearsona. Wartości dodatnie oznaczają, że wraz ze wzrostem jednej zmiennej rośnie druga, a wartości ujemne – że wzrost jednej wiąże się ze spadkiem drugiej. Należy podkreślić, że korelacja opisuje współwystępowanie i nie oznacza związku przyczynowo-skutkowego.

Najsilniejsza dodatnia zależność występuje pomiędzy Calories_Burned i Session_Duration (r = 0,908), co jest intuicyjne: dłuższy czas trwania sesji jest związany z większą liczbą spalonych kalorii. Bardzo wysoka korelacja pojawia się również pomiędzy BMI i Weight (r = 0,853) – przy zbliżonym wzroście wyższa masa ciała przekłada się na wyższe BMI.

Umiarkowanie silna dodatnia zależność dotyczy Session_Duration i Workout_Frequency (r = 0,644), co sugeruje, że osoby ćwiczące częściej w tygodniu mają tendencję do dłuższych jednostek treningowych. W podobnym kierunku działa zależność Calories_Burned i Workout_Frequency (r = 0,576) – częstsze treningi wiążą się średnio z większym wydatkiem energetycznym.

Wśród najsilniejszych korelacji ujemnych dominują związki z Fat_Percentage. Zmienna ta jest ujemnie powiązana ze spalonymi kaloriami (Calories_Burned vs Fat_Percentage, r = -0,598), a także z czasem sesji (Fat_Percentage vs Session_Duration, r = -0,582) oraz częstotliwością treningów (Fat_Percentage vs Workout_Frequency, r = -0,537). Oznacza to, że w tym zbiorze osoby z wyższym odsetkiem tkanki tłuszczowej mają tendencję do krótszych i rzadszych treningów oraz niższego wydatku energetycznego. Warto także zauważyć ujemną zależność między Fat_Percentage i Water_Intake (r = -0,589), wskazującą, że większy udział tkanki tłuszczowej wiąże się przeciętnie z mniejszym spożyciem wody.

Słabsze, ale dodatnie korelacje pojawiają się między Water_Intake i Weight (r = 0,394) oraz między Height i Water_Intake (r = 0,394), co może sugerować, że osoby o większych gabarytach (wyższe/cięższe) piją nieco więcej wody – jednak siła tych zależności jest już umiarkowana.

Podsumowując, największe i najbardziej „oczywiste” powiązania dotyczą relacji między parametrami treningu (czas sesji i spalone kalorie) oraz między masą ciała i BMI. Z kolei zmienna Fat_Percentage jest konsekwentnie ujemnie skorelowana z miarami aktywności i wydatku energetycznego, co może wskazywać na różnice w stylu treningu i ogólnej kondycji w analizowanej próbie.
...
```{r message=FALSE, warning=FALSE}
# (opcjonalnie) instalacje jednorazowe:
# install.packages(c("broom", "car", "emmeans"))

library(dplyr)
library(broom)    # porządne wyświetlanie wyników
library(car) # Anova(type=2) do modeli z czynnikami
library(emmeans)  # porównania post-hoc w modelach
...
Cel: Sprawdzić, czy segmentacja użytkowników oparta na budowie ciała (BMI i % tłuszczu) różnicuje ich również pod względem nawyków treningowych i cech demograficznych.

Metoda: Zastosowano algorytm k-średnich ($k=3$) do podziału grup, a następnie wykorzystano test ANOVA Welcha z korektą Holma dla zmiennych liczbowych oraz testy chi-kwadrat dla zmiennych jakościowych.
...
Zastosowano segmentację metodą k-średnich (k=3) na podstawie BMI oraz procentu tkanki tłuszczowej. Następnie sprawdzono, czy wyodrębnione klastry różnią się także innymi cechami. Test ANOVA Welcha z korektą Holma wykazał istotne różnice między klastrami dla czasu trwania sesji, spalonych kalorii oraz częstotliwości treningów (p_holm < 0.05). Nie stwierdzono natomiast istotnych różnic dla wieku ani parametrów tętna (Resting_BPM, Avg_BPM). Dodatkowo testy chi-kwadrat wykazały zależność klastra od płci oraz poziomu doświadczenia, natomiast nie wykazały zależności od typu treningu. Oznacza to, że segmentacja na podstawie BMI i Fat% rozróżnia grupy, które różnią się również zachowaniami treningowymi, a nie tylko budową ciała.
...
Analiza czynników wpływających na średnie tętno podczas treningu została przeprowadzona przy użyciu regresji liniowej wielorakiej, co pozwoliło na ocenę wpływu poszczególnych zmiennych przy jednoczesnej kontroli pozostałych parametrów fizycznych i treningowych. Zbudowany model okazał się bardzo dobrze dopasowany do danych R^2 = 0.8035, skorygowane R^2 = 0.8007, co oznacza, że uwzględnione zmienne wyjaśniają aż 80% całkowitej zmienności parametru Avg_BPM.
Wyniki modelu wskazują, że kluczowymi i statystycznie istotnymi predyktorami średniego tętna są czas trwania sesji, liczba spalonych kalorii, wiek oraz płeć użytkownika. Odnotowano silny, ujemny związek z czasem trwania treningu beta = -92.79, p < 0.001, co sugeruje, że w bardzo długich sesjach średnie tętno ma tendencję do stabilizowania się na niższych poziomach w porównaniu do krótkich, intensywnych zrywów. Zgodnie z oczekiwaniami, liczba spalonych kalorii wykazuje dodatnią zależność z tętnem beta = 0.129, p < 0.001, podobnie jak wiek badanego beta = 0.392, p < 0.001, co może wskazywać na wyższy koszt fizjologiczny wysiłku u osób starszych. Co istotne, stwierdzono wyraźne różnice między płciami – mężczyźni charakteryzują się średnim tętnem niższym o ok. 11.61 uderzeń na minutę względem kobiet przy zachowaniu tych samych parametrów treningu p < 0.001. Pozostałe zmienne, w tym BMI, procent tkanki tłuszczowej oraz rodzaj treningu, nie okazały się istotnymi predyktorami w tym modelu, co sugeruje, że ich wpływ na tętno jest wtórny wobec dynamiki samej sesji i podstawowych cech demograficznych.
...
Weryfikacja związku między nawodnieniem a wydajnością treningową objęła analizę korelacji rang Spearmana oraz budowę modelu regresji liniowej kontrolującego czas trwania i rodzaj aktywności. Wstępna analiza wykazała istotną statystycznie, dodatnią i umiarkowaną korelację między ilością wypitej wody a liczbą spalonych kalorii ($\rho = 0.346, p < 0.001. Jednocześnie nie stwierdzono żadnego istotnego związku między nawodnieniem a średnim tętnem podczas ćwiczeń rho = -0.0057, p = 0.858, co wskazuje, że spożycie wody nie wpływa bezpośrednio na intensywność pracy serca, lecz na inne aspekty wydatku energetycznego.
W celu sprawdzenia, czy zależność między wodą a kaloriami nie wynika jedynie z faktu, że dłuższe treningi wymuszają większe picie, zastosowano regresję wieloraką. Model potwierdził, że Water_Intake pozostaje niezależnym i istotnym predyktorem liczby spalonych kalorii beta = 49.19, p < 0.001. Przy założeniu stałego czasu trwania treningu, zwiększenie spożycia wody o jeden litr wiąże się średnio ze wzrostem wydatku energetycznego o ok. 49 kcal. Tak silna zależność, utrzymująca się nawet po wyeliminowaniu wpływu czasu trwania sesji R^2 = 0.836, pozwala na sformułowanie wniosku, że odpowiednie nawodnienie jest statystycznie powiązane z wyższą efektywnością treningową, choć mechanizm ten nie jest zapośredniczony przez wzrost tętna.
